                ...
                case ANNOUNCEMENT:
                {
					...
					cout<<"Processing Announcement Packet pushed"<<endl;
					  ...
					  cout<<"\n Accq mutex\n";
						 ...
						 list<RTrecord>::iterator rtindex_end = RoutingTable.end();
						 ...
						 while(rtindex_ != RoutingTable.end())
						 {
							 ...
							 if( rtindex_->cID ==  record_test_rt.cID )
							 {
								 ...
								 //check for hops
								 if(rtindex_->num_hops   < record_test_rt.num_hops   )
								 {
									 //Keep this record and its interface
									 record_test_rt = *rtindex_;
									 record_test_rt.ttl_rt = g_ROUTING_TABLE_EXPIRY_TIME;
								 ...
								 //Erase
								 cout<<"\n Erase"<<endl;
								 RoutingTable.remove(*rtindex_);
								 cout<<"\n EraseD"<<endl;
								 //rtindex_end = RoutingTable.end();
								 //cout<<"\n got new end "<<endl;

							 }

							 rtindex_++;
							 cout<<"\n incremented rtindex_++"<<endl;
						 }
						 cout<<"\n push back"<<endl;
					 ...
					 cout<<"\n Forwardig the announcement \n";
						for(int index=0; index < num_RX_TX_RemoteInterfaces; index++)
						{
						  _recvPacket->accessHeader()->setOctet((unsigned char)(hops+1),HOSTID_OR_HOPS_POS);
							...
							if(index != interface)
							{
							 cout<<" Broadcasting announcement through sending interface "<< index <<endl;
