...
#define MAX_HEADER_SIZE 4 // edited
...
public:
	 ...
	 */
	inline void init() {
		memset(info_, 0, MAX_HEADER_SIZE);
	}
	/**
	 ...
	 */
	inline unsigned char* accessInfo() {
		return info_;
	}
	/**
	 ...
	 */
	inline void setOctet(unsigned char c, int position) {
		*(info_ + position) = c;
		length_++;
	}
	/**
	 * Set the header size
	 */
	inline void setHeaderSize(int len) {
		length_ = len;
	}
	/**
	 * get an octet
	 */
	inline unsigned char getOctet(int position) {
...
public:
	...
	/// get the MAC address
	inline unsigned char* getHWAddr() {
		return macaddr_;
	}
...
protected:
	...
	inline void setHostname(const char* hostname) {
		myaddr_.setHostname(hostname);
	}
	/// set port of local address
	inline void setPort(const short port) {
		myaddr_.setPort(port);
	}
	/// set hostname of remote address
	inline void setRemoteHostname(const char* hostname) {
		itsaddr_.setHostname(hostname);
	}
	/// set port no of a remote address
	inline void setRemotePort(const short port) {
		itsaddr_.setPort(port);
	}
...
public:
  ...
  inline void timerHandler()
  {
    if (!ackflag_)
      {
       int ContentID = (int)lastPkt_->accessHeader()->getOctet(1);
       std::cout << "\n The request for "<<ContentID<<" has not been acknowledged yet. Re-send request..." <<std::endl;

       sendPacket(lastPkt_);
       //schedule a timer again
       timer_.startTimer(ARQ_TIME);
      }
  }
private:
  bool ackflag_;
